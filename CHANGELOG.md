...
- **`brace-expansion` override**: forcing `^5.0.8` under `minimatch@3` was a live TypeError on any brace glob (v5 exports `{ expand }`, v1 was callable — only non-brace code paths kept `serve`/`pa11y-ci` working). Replaced with `vendor/brace-expansion-compat`: the patched v5.0.8 algorithm (expansion caps verified) behind a callable CJS shim. `npm audit`: 0; brace globs verified working — `*.{css,js}` matches and `{1..10^10}` clamps instead of OOM.
