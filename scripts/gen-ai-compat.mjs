...
// Tailwind v3 AND Bootstrap hosts. farvist-ai.css is emitted entirely in @layer; Tailwind
...
// `border-radius` is not something Tailwind's preflight zeroes — it is here for
// the OTHER host class this file now serves: a page whose own un-layered CSS
// styles `.btn` (Bootstrap does), which overrides the kit's layered radius too.
const CONTROL = ['padding', 'font-size', 'font-weight', 'line-height', 'color', 'background-color', 'border', 'border-radius'];
const SPEC = [
  ...
  ['.snippet-header', ['border', 'border-bottom']],
  ['pre.snippet', ['border']],
  ['.prose pre', ['border']],
  ...
  // --- MUST STAY LAST -------------------------------------------------------
  // Everything above is emitted un-layered, which also makes it beat the kit's
  // own LAYERED refinements of the same properties. These re-assert the ones
  // that carry meaning: without them a Tailwind v3 host renders an error toast
  // identically to a success toast. Equal specificity to the shorthands above
  // (.toast-success vs .toast), so source order is what decides — keep these at
  // the end of the SPEC, since the emitter preserves this order.
  ['.toast-success', ['border-left-color']],
  ['.toast-danger', ['border-left-color']],
  ['.toast-warning', ['border-left-color']],
  ['.toast-info', ['border-left-color']],
  ['.tool-call-done', ['border-left-color']],
  ['.tool-call-error', ['border-left-color']],
  ['.message-out .message-bubble', ['border-color']],
  ['.prompt:focus-within', ['border-color']],
];
...
const banner = `/*! Farvist AI · Tailwind v3 + Bootstrap compat — GENERATED by scripts/gen-ai-compat.mjs, do not edit.
 ...
 * Load it on EVERY Bootstrap host too: the kit's own buttons carry class="btn", and
 * Bootstrap's un-layered .btn overrides their padding, weight, radius and glass chrome.
 * Tailwind v4 preflight is layered and needs nothing. */\n`;
let body = '';
